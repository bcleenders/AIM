...
object Trends extends App {
  ...
  val corpus: RDD[Item] = sc.wholeTextFiles("articles/HN-stories-2015-April-*").flatMap { case (_, file) =>
...
//  println("Number of empty articles: ", corpus.filter(_.webpage.cleanedText == "").count())
  ...
  //val tokenized = lemmatize(corpus.map(_.webpage.cleanedText), true)

  val tokenized = corpus.map(_.webpage.cleanedText).mapPartitions( partition => {
  ...
  val outTM = new java.io.FileWriter("output/Topic Matrix_April2015.txt")
  var counter = 0
  ...
  val outTD = new java.io.FileWriter("output/Topic Distributions_April2015.txt")
