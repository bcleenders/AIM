...
object Lemmatize extends App {
  ...
  val corpus: RDD[Item] = sc.wholeTextFiles(s"$path/output_awesome/HN-stories-" + jobQuery).flatMap { case (_, file) =>
  ...
  tokenized.saveAsTextFile(s"$path/lemmatized/")
}
