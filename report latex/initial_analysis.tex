...
To give a formal definition: let $S_m$ the collection of all stories in month $m$, $T_i$ all stories in the topic number $i$, let $s_u$ (resp. $s_c$) be the number of upvotes (resp. comments) story $s$ has received. Then, given a topic id $i$ and a month $m$, the score for that topic in that month is:

$$
	score(i, m) = 
		\frac{1}{3} \frac{|\{s \in S_m \cup T_i\}|}{|\{s \in S_m\}|} + 
